...
def model_fourier3(ucoord, vcoord, theta1, inc1, c, s, la, lkr, flor, fs, fd, fo,c2,s2):
    #pdb.set_trace()
    ...
    for i in range(ucoord.shape[0]):
        ...
        skeleton = special.j0(2 * np.pi * r_uvti1 * ar) + (-1j) ** (1) * phasor[0] * np.cos(
            1 * np.deg2rad((phasor_uv[1] - phasor[1]))) * \
                   special.j1(2 * np.pi * r_uvti1 * ar) + (-1j)**2 * phasor2[0] * np.cos(2*np.deg2rad((phasor_uv[1] - phasor2[1])))*special.jn(2,2*np.pi*r_uvti1*ar) #+ (-1j)**2 * phasor3[0] * np.cos(2*np.deg2rad((phasor_uv[1] - phasor3[1])))*special.jn(3,2*np.pi*r_uvti1*ar)
...
def model_im3(im_size, scale, theta1, inc1, c, s, la, lkr, flor, fs, fd, fo, c2, s2, wave, year):
    sz = 1.0 * im_size
    if sz % 2 == 0:
