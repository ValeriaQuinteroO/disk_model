...
def lnprior(param): #Function Log of prior distribution -> Insert the range of the parameters
    ...
    if ( 150 < theta1 < 245) and ( 1 < incl < 60 ) and (-3.0 < s1 < -1.1) and (-1.4 < c1 < -0.6)  and (1.0 < la < 1.3) and \
...
if __name__ == "__main__":
    ...
    year = '2018_205_1mod_chi_intentoo'
...
#Prior parameters uniform distribution EMCEE
    ...
    nsteps = 6000
    pos1 = np.random.uniform(150,245, size=nwalkers) #theta1 (position angle)
    pos2 = np.random.uniform(1, 60, size=nwalkers)  #incl (inclination)
    pos3 = np.random.uniform(-1.4, -0.6, size=nwalkers) #C1 (cosine of the modulation)
    ...
    fig.savefig("205_test_2018_1mod_chi_intentoo.png")
    ...
    fig2.savefig("205_samples_2018_1mod_chi_intentoo.png")
...
##########
    ...
    fig3.savefig('model_azimuth_2018_205_1mod_chi_intentoo.pdf', bbox_inches='tight')
    plt.show()
    pdb.set_trace()
