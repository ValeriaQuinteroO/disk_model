...
oi_data = '205_COMB_FT_RCra.fits'

ims = ['fft_model_2.0579_2019_205_2mod_cor.fits']
...
for i in range(len(ims)):
    ...
    uv_range = np.sqrt(u ** 2 + v ** 2) / waves
    ax1.errorbar(uv_range, vis2, yerr=vis2_err, fmt='o', color='black')
    ax1.plot(uv_range, (vis_synth) ** 2, 'o', color='red', zorder=500, alpha=0.8,  label = str(np.round(waves/1e-6, 3))+' $\mu$m')
    ax2.errorbar(uv_cp / waves, t3, yerr=0.3, fmt='o', color='black')
    ax2.plot(uv_cp / waves, t3phi_synth, 'o', color='red', zorder=500, alpha=0.8)
    #ax3.plot(uv_range, (vis2 - (vis_synth ** 2)), 'o', color='red')
    #ax4.plot(uv_cp / waves, (t3 - t3phi_synth), 'o', color='red')
    mean_res_vis = np.mean((vis2 - (vis_synth ** 2)))
    mean_res_cp = np.mean((t3 - t3phi_synth))
    std_res_vis = np.std((vis2 - (vis_synth ** 2)))
    std_res_cp = np.std((t3- t3phi_synth))
