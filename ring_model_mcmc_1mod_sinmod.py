...
def lnlike (param, u, v, v2=None, v2err=None, cp=None, cperr=None, sta_index_v=None, sta_index_cp=None, \
    ...
    theta1, incl, la, lkr, fs, fd = param  #Order with POS1,2,...
    ...
    for i in range(len(wave)):
        ...
        vis = mod.model_fourier3(sf_u[:, i], sf_v[:, i], theta1, incl, 0, 0, la, lkr, 1.0, fs, fd, (1-fs-fd)) #Llama al modelo de disco
...
def lnprior(param): #Function Log of prior distribution -> Insert the range of the parameters
    theta1, incl, la, lkr, fs, fd = param
    if ( 150 < theta1 < 190) and ( 30 < incl < 60 )  and (0.0 < la < 1.05) and \
            (-2.1 < lkr < 0.5) and (0 < (fs + fd) <= 1) and (fd > 0) and (fs > 0):
...
if __name__ == "__main__":
    ...
    oi_file = 'RCra/FT_DATA/2016_09_209_COMB_RCra.fits'
    year = '2016_06_209_sinmod_intento'
...
#Prior parameters uniform distribution EMCEE
    ...
    ndim, nwalkers = 6, 100 #Parameters and MC
    nsteps = 2000
    pos1 = np.random.uniform(150, 190, size=nwalkers) #theta1 (position angle)
    pos2 = np.random.uniform(30, 60, size=nwalkers)  #incl (inclination)
    ...
    pos6 = np.random.uniform(-2, 0.5, size=nwalkers)  # lkr (log of the kernel size)
    ...
    pos = np.array([pos1, pos2, pos5, pos6, pos7, pos8]).T
    ...
    # 'c2', 's2'
    # c2_mcmc[0], s2_mcmc[0]]
    theta1_mcmc, incl_mcmc, la_mcmc, lkr_mcmc, fs_mcmc, fd_mcmc = map(lambda v: (v[1], v[2] - v[1], v[1] - v[0]), #, c2_mcmc, s2_mcmc
                                   zip(*np.percentile(samples, [16, 50, 84], axis=0)))
    fig = corner.corner(samples, labels=['theta', 'incl', 'la', 'lkr', 'fs', 'fd'], \
                        show_titles=True, title_fmt="0.2e",
                        levels=(1 - np.exp(-0.5), 1 - np.exp(-2.0)), \
                        truths=[theta1_mcmc[0], incl_mcmc[0], la_mcmc[0], \
    ...
    fig.savefig("209_test_2016_06_sinmod_intento.png")

    fig2, axes = plt.subplots(10, figsize=(10, 7), sharex=True)
    labels = ['theta', 'incl', 'la', 'lkr', 'fs', 'fd'] #'c2', 's2'
    for i in range(ndim):
        ax = axes[i]
    ...
    fig2.savefig("209_samples_2016_06_sinmod_intento.png")
...
##########
    ...
    for i in range(len(wave)):
        vis = mod.model_fourier3(uur / wave[i], vvr / wave[i], theta1_mcmc[0], incl_mcmc[0], 0, 0, \
                                 la_mcmc[0], lkr_mcmc[0], 1.0, \
                                fs_mcmc[0], fd_mcmc[0], 1-fs_mcmc[0]-fd_mcmc[0])
        im = mod.model_im3(512, 1.0 / mod.mas2rad(0.1 * 512), theta1_mcmc[0], incl_mcmc[0], 0, 0, \
                                 la_mcmc[0], lkr_mcmc[0], 1.0, \
        ...
        if len(wave) == 1:
            ...
            ax1.errorbar(rr[ind_v2], vis2[ind_v2], yerr=vis2_err[ind_v2], fmt='o', color='black')
            ax1.plot(rr[ind_v2], visamp[ind_v2] ** 2, 'o', color='red', zorder=200,
                     label=str(np.round(wave[i] / 1e-6, 4)) + ' $\mu$m')
            ax2.errorbar(observables['uv_cp'][ind_cp], cp[ind_cp], yerr=cp_err[ind_cp], fmt='o', color='black')
            ax2.plot(observables['uv_cp'][ind_cp], cp_mod[ind_cp], 'o', color='red', zorder=200)
            ax3.plot(rr[ind_v2], (vis2[ind_v2] - np.squeeze(visamp[ind_v2] ** 2)), 'o', color='red')
            ax4.plot(observables['uv_cp'][ind_cp], (cp[ind_cp] - np.squeeze(cp_mod[ind_cp])), 'o', color='red')
        else:
            [ind_v2] = np.where(flag_vis2[:, i] == False)
            [ind_cp] = np.where(flag_cp[:, i] == False)
            # colorVal = scalarMap.to_rgba(values[i])
            ax1.errorbar(rr[ind_v2], vis2[ind_v2, i], yerr=vis2_err[ind_v2, i], fmt='o', color='black')
            ax1.plot(rr[ind_v2], visamp[ind_v2] ** 2, 'o', color='red', zorder=200,
                     label=str(np.round(wave[i] / 1e-6, 4)) + ' $\mu$m')
            ax2.errorbar(observables['uv_cp'][ind_cp, i], cp[ind_cp, i], yerr=cp_err[ind_cp, i], fmt='o', color='black')
            ax2.plot(observables['uv_cp'][ind_cp, i], cp_mod[ind_cp], 'o', color='red', zorder=200)
            ax3.plot(rr[ind_v2], (vis2[ind_v2, i] - np.squeeze(visamp[ind_v2] ** 2)), 'o', color='red')
            ax4.plot(observables['uv_cp'][ind_cp, i], (cp[ind_cp, i] - np.squeeze(cp_mod[ind_cp])), 'o', color='red')
    ...
    fig3.savefig('model_azimuth_2016_06_209_sinmod_intento.pdf', bbox_inches='tight')
    plt.show()
    pdb.set_trace()
